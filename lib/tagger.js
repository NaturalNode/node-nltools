...
HunposTagger = function(path_to_model,path_to_bin,callback) {
  ...
  this._encoding = HunposTagger._hunpos_charset;
	...
	fs.stat(path_to_model,function(error,file){
		...
		_this._hunpos = spawn(_this._hunpos_bin, [_this._hunpos_model]);
		
		_this._hunpos.stderr.on('data', function (data) {			
...
// n-grams might work better here.
// types are added by NER
...
// Return an array of just the POS data
HunposTagger.prototype.getPOS = function() {
	var p = [];
	_(this.tags).each(function(t){p.push(t.pos)});
	return p;
}
